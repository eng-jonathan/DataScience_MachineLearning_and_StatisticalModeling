...
```{r}
mat = matrix(data = NA, nrow = ncol(X), ncol = ncol(X))
colnames(mat) = colnames(X)
first_col =lm(y ~ mat[,1])
first_col
...
```{r}
diamonds$color = ifelse(diamonds$color == "G", 1, 0)
simple_lm = lm(color ~ y, diamonds)
...
The features are likely irrelevant

Model `price` with both `color` and `clarity` with and without an intercept and report the coefficients.

```{r}
mod = lm(price ~ color + clarity, diamonds)
coef(mod)
summary(mod)$r.squared 

mod = lm(price ~ 0 + color + clarity, diamonds)
coef(mod) 
summary(mod)$r.squared 
```
...
All coefficents changed because with y intercept becomes the reference variable, without the intercept there is no longer a reference variable. But the values of no intercept coefficents can be achieved by adding the intercept to the corresponding coeficcients of each variable, and vice versa. 
...
```{r}
m = matrix( c(1, 1, runif(2)), nrow=2)
x = m[, 1]
y = m[, 2]

theta= abs( acos(x %*% y / sqrt( sum(x**2) * sum(y**2) )))
...
The absolute angle is converging to 90 degrees, which makes sense as we add additional dimensions.

Create a vector $y$ by simulating $n = 100$ standard iid normals. Create a matrix of size 100 x 2 and populate the first column by all ones (for the intercept) and the second column by 100 standard iid normals. Find the $R^2$ of an OLS regression of `y ~ X`. Use matrix algebra.

```{r}
mat = cbind(1, matrix(runif(100)))
x = mat[, 2]
y = mat[, 1]
Xmm = model.matrix(y ~ x)
...
for (i in 98){
  n = rnorm(100)
  Xmm = cbind(Xmm,n) 
  r_squ[i] = summary(lm(y ~ Xmm))$r.squared
}
r_squ
```
...
q
```
Rsquared is continuously increasing and goes beyond 100 due to the columns being more than 100.
